...
function initPlayerEngine() {
  ...
  shakaPlayerInstance.configure({
    streaming: { bufferingGoal: 30, rebufferingGoal: 10, bufferBehind: 15 }
  });
...
function switchVideoQuality(heightValue) {
  ...
  if(heightValue === "auto") { shakaPlayerInstance.configure({ abr: { enabled: true } }); }
  else {
...
// ربط أحداث الريموت الماجيك بالاتجاهات الأربعة والـ OK وتقليب القنوات الحية
document.addEventListener("keydown", (e) => {
  triggerOSDVisibility();
  if (e.key === "Enter" || e.key === "Ok" || e.key === " ") { togglePlayPauseState(); e.preventDefault(); }
  if (e.key === "ArrowLeft") { video.currentTime -= seekDuration; }
  if (e.key === "ArrowRight") { video.currentTime += seekDuration; }
  
  // أزرار تقليب القنوات لريموت LG (ChannelUp / ChannelDown)
  if (e.key === "ChannelUp" || e.key === "PageUp" || e.key === "ArrowUp") { video.currentTime += 60; }
  if (e.key === "ChannelDown" || e.key === "PageDown" || e.key === "ArrowDown") { video.currentTime -= 60; }
...
window.onload = function() {
  ...
  document.getElementById('player-html').className = activeTheme; initPlayerEngine();
};
