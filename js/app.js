...
// المحرك الحركي المستوحى من نظام جلب السيرفرات الحقيقي لبناء روابط البث مباشرة
function generateServerPlaylistContent() {
  const isAr = currentLang === "ar";
  
  // جلب بيانات آخر سيرفر تم إدخاله بواسطة المستخدم
  ...
  // استخدام بيانات السيرفر النشط لبناء روابط البث المشفرة الحقيقية ديناميكياً
  const activeServer = savedPlaylists[savedPlaylists.length - 1];
  const host = activeServer.url;
  const user = activeServer.user;
  const pass = activeServer.pass;

  // 1. توليد قنوات البث المباشر بروابط السيرفر الحقيقية
  liveChannels = [
    ...
    { stream_id: 203, name: isAr ? "قناة أبوظبي الرياضية" : "AD Sports HD", category_name: "Sports", stream_icon: "https://placehold.co/400x540/00b4d8/ffffff?text=AD+Sports", url: `${host}/live/${user}/${pass}/203.m3u8`, type: "live" }
  ];

  // 2. توليد الأفلام بروابط السيرفر الحقيقية (صيغة mp4/mkv الشائعة في Xtream)
  moviesList = [
    ...
    { stream_id: 302, name: isAr ? "فيلم الأنمي ودموع الغزال" : "Big Buck Bunny Premium", category_name: "Animation", stream_icon: "https://placehold.co/400x540/e50914/ffffff?text=Buck+Bunny", url: `${host}/movie/${user}/${pass}/302.mkv`, type: "movie" }
  ];

  // 3. توليد المسلسلات بروابطها وحلقاتها الديناميكية
  seriesList = [
...
function saveIPTVServer() {
  ...
  // إزالة الشرطة المائلة الزائدة من نهاية الرابط لضمان دقة مسار الـ URL
  if (url.endsWith('/')) {
    url = url.slice(0, -1);
  }
  ...
  // تفعيل السحب الفوري وإعادة بناء الروابط الحية
...
function renderVerticalEpisodes(series) {
  ...
  const mockEpisodes = [
    { id: "ep_1", title: "الحلقة 1: بداية مسار البث الملحمي", desc: "تستعرض الحلقة فك تداخل الشاشات والـ CORS وضخ قنوات السيرفر بكفاءة 4K سينمائية عريضة.", url: `${activeServer.url}/series/${activeServer.user}/${activeServer.pass}/401_1.mp4`, thumb: "https://placehold.co/640x360/1f6feb/ffffff?text=EP+1" },
    { id: "ep_2", title: "الحلقة 2: المعالجة الحركية وأزرار التلفزيون", desc: "تفعيل التحكم المنفصل للاتجاهات الأربعة يمين ويسار وفوق وتحت ومؤشر الماوس السحري.", url: `${activeServer.url}/series/${activeServer.user}/${activeServer.pass}/401_2.mp4`, thumb: "https://placehold.co/640x360/00c851/ffffff?text=EP+2" }
  ];
