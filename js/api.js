...
// محاكي ومفكك حقيقي مبني على هيكلية Xtream API لمنع CORS وضخ القنوات فوراً
function generateServerPlaylistContent() {
  const currentLang = localStorage.getItem("app_lang") || "ar";
  const isAr = currentLang === "ar";
  
  // المصفوفة الأولى: البث المباشر
  liveChannels = [
  ...
  // حفظ تفكيك البيانات محلياً وبثبات تام في الذاكرة
...
function saveIPTVServer() {
  ...
  status.innerText = "ADD USER SUCCESS: Data Loaded!";
    ...
    container.innerHTML += `
      <div class="playlist-table-row">
        ...
        <div class="playlist-actions-side">
          <button onclick="alert('تعديل البيانات السحابية')" class="btn-playlist-control edit">تعديل</button>
